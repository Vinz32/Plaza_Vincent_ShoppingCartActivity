...
class Program
{
    ...
    static void AddToCart(Product selected, int qty)
    {
        for (int i = 0; i < cartCount; i++)
        {
            ...
            {
                cartQty[i] += qty;
                cartSub[i] += selected.GetTotal(qty);
                selected.Deduct(qty);
        ...
        selected.Deduct(qty);
        Console.WriteLine("Added to cart: " + selected.Name + " x" + qty + " = P" + cartSub[cartCount]);
        cartCount++;
    }
    ...
    static void FilterByCategory()
    {
        ...
        for (int i = 0; i < menu.Length; i++)
        {
            ...
            for (int j = 0; j < catCount; j++)
                if (cats[j] == menu[i].Category) { exists = true; break; }
            if (!exists)
                cats[catCount++] = menu[i].Category;
        }
        ...
        for (int i = 0; i < menu.Length; i++)
            if (menu[i].Category == selectedCat)
                menu[i].Display();
    }
    ...
    static void ViewCart()
    {
        ...
        for (int i = 0; i < cartCount; i++)
        {
            ...
            Console.WriteLine("[" + (i + 1) + "] " + p.Name + " x" + cartQty[i] + " = P" + cartSub[i]);
            total += cartSub[i];
        }
        Console.WriteLine("─────────────────");
    ...
    static void UpdateCartItem()
    {
        ...
        {
            Console.WriteLine("Not enough stock. Only " + p.Stock + " available.");
            return;
        }
        ...
        cartSub[index] = p.Price * newQty;
        Console.WriteLine("Updated: " + p.Name + " x" + cartQty[index] + " = P" + cartSub[index]);
    }
    ...
    static void RemoveAtIndex(int index)
    {
        ...
        Console.WriteLine("Removed: " + p.Name + " from cart.");
    ...
    static bool Checkout()
    {
        ...
        {
            Product p = FindProduct(cartId[i]);
            Console.WriteLine(p.Name + " x" + cartQty[i] + " = P" + cartSub[i]);
        }
    ...
    static void ShowLowStockAlert()
    {
        ...
        for (int i = 0; i < menu.Length; i++)
        {
            if (menu[i].Stock <= 5)
            {
                Console.WriteLine("⚠  " + menu[i].Name + " has only " + menu[i].Stock + " stock(s) left!");
                hasLow = true;
            }
        }
